...
```
\
As the data shown, there's a lot of N/A value in some  column. We remove the column which have NA value more than 50%
```{r }
...
```
\
There still a lot of column in the dataframe then we use PCA method to reduce the dimension (with 95% threshold)
```{r }
# PCA
...
```
\
Next, we remove non-related column from the dataframe
```{r }
...
```
\
We split the dataframe into the training set and the other one for the test set.
```{r }
# Split the data
set.seed(33933)
...
Create 10-fold cross validation to control the model
```{r }
# Create 10-fold cross validation
train.control <- trainControl(method = "cv", number = 10, verboseIter = TRUE)
```
\
Create a random forest model prediction
```{r cache=TRUE}
